...
class MyDataset(Dataset):
    ...
    def read_data(self):
        if os.path.exists(self.data_path + 'train') == False:
            print('===== Reforming Data... =====')
            self.reform_data('train')
            self.reform_data('test')
        ...
        if self.task != 'all':
            ...
            coarse_labels_needed = fine_labels_needed   # In StanfordDogs we treat 'coarse' same to 'fine' 
        else:
    ...
    def reform_data(self, flag):
        if flag == 'train':
            split = scipy.io.loadmat(os.path.join(self.data_path, 'train_list.mat'))['annotation_list'] 
            labels = scipy.io.loadmat(os.path.join(self.data_path, 'train_list.mat'))['labels']
        elif flag == 'test':
            split = scipy.io.loadmat(os.path.join(self.data_path, 'test_list.mat'))['annotation_list']
            labels = scipy.io.loadmat(os.path.join(self.data_path, 'test_list.mat'))['labels']

        split = [item[0][0] for item in split]
        labels = [item[0]-1 for item in labels]
        images = list(zip(split, labels))
        data = [(annotation+'.jpg', idx) for annotation, idx in images]
        saves={}
        data_list=[]
        label_list=[]
        ...
        with open(self.data_path+flag, 'wb') as fp:
...
if __name__ == '__main__':
    data_path = GV.dataset_path_prefix + 'StanfordDogs' + '/'
    n_workers = GV.WORKERS
    split = 'train'
    augmentation = 'simple'
    batch_size = 2

    task_list_file_path =  GV.dataset_path_prefix + 'StanfordDogs' + '/task_lists/' + \
        '_minc=0_maxc=119.repo'
